...
public:
    ...
    void setInstructionNumber(int number) {
        Module::instructionNumber = number;
    }
...
char *readSymbol(ifstream &inputStream) {
    char *tok = getToken(input_stream);
...
void Pass1(ifstream &inputStream) {
    ...
    while (!input_stream.eof()) {
        ...
        afterDef = false;
        int defcount = readInt(input_stream);
        if (defcount == -1) {
            break;
        }
        if (defcount > 16) {
        ...
        afterDef = true;
        int use_count = readInt(input_stream);
        if (use_count == -1) {
            break;
        }
        if (use_count > 16) {
        ...
        afterDef = true;
        int instructionCount = readInt(input_stream);
...
void Pass2(ifstream &inputStream) {
    ...
    while (!input_stream.eof()) {
        ...
        int instNum;
        afterDef = false;
        int definition_count = readInt(input_stream);
        ...
        afterDef = true;
        int instance_count = readInt(input_stream);
        if (instance_count == -1) {
            break;
        }
...
void initializePass2(vector<Symbol> &useList, vector<Symbol *> &symbolTable, int &currentBase) {
    symbolTable = module.getSymbolTable();
    currentBase = module.getCurrentBase();
